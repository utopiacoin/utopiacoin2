...
*Setup instructions for a gitian build of Newcoin using a Debian VM or physical system.*
...
- [Building Newcoin](#building-newcoin)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-newcoin-host/README.md).
...
Clone the git repositories for newcoin and gitian and then checkout the newcoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/newcoin/newcoin
cd newcoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the newcoin repository
...
To build Newcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the newcoin repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/newcoin/newcoin
...
For example:
```bash
URL=https://github.com/laanwj/newcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/newcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/newcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/newcoin-build.assert
```
...
[newcoin/gitian.sigs](https://github.com/newcoin/gitian.sigs/) repository, or if that's not possible create a pull
