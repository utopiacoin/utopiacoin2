...
# Get BUILDDIR and REAL_NEWCOIND
. "${CURDIR}/tests-config.sh"

export NEWCOINCLI=${BUILDDIR}/qa/pull-tester/run-newcoin-cli
export NEWCOIND=${REAL_NEWCOIND}
...
if [ "x${ENABLE_NEWCOIND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and newcoind must all be enabled"
fi
