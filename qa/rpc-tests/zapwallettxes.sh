...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars NEWCOIND and NEWCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

NEWCOIND=${NEWCOIND:-${1}/newcoind}
CLI=${NEWCOINCLI:-${1}/newcoin-cli}
...
$NEWCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$NEWCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
